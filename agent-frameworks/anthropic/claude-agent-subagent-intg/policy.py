"""The deterministic local policy tool used only by the research subagent."""
...
def load_policy(policy_id: str) -> dict[str, Any] | None:
    """Read the local fixture; this lab makes no external policy request."""
    return json.loads(FIXTURE_PATH.read_text(encoding="utf-8")).get(policy_id)
...
async def lookup_policy(arguments: dict[str, Any]) -> dict[str, Any]:
    """Return the Claude SDK in-process MCP response format."""
