"""Run the Claude policy-validation workflow with optional BHANUJ evidence."""
...
from governance import create_policy_hooks, create_reporter, new_session_id
...
async def run() -> tuple[str, str, str | None, str]:
    ...
    options = create_policy_options(model, create_policy_hooks(reporter))
...
def _evidence_status(reporter: object) -> str:
    # Kept separate so terminal output only reports delivery health, not evidence.
    enabled = getattr(reporter, "enabled")
    execution_id = getattr(reporter, "execution_id")
    delivery_failed = getattr(reporter, "delivery_failed")
    if not enabled:
        return "disabled"
    if execution_id and not delivery_failed:
        return "delivered"
